...
# CODEX UPGRADE PROTOCOL — via-decide/VIA
# Append this to: .codex/instructions.md
# This section governs ALL upgrade tasks — new features, refactors,
# dependency bumps, UI changes, social layer additions, anything that
# touches an already-working file.

════════════════════════════════════════════
UPGRADE RULE 0 — DEFINE THE UPGRADE FIRST
════════════════════════════════════════════

Before writing a single line, output this block:

  UPGRADE PLAN
  ├── Target files: <list every file to be touched>
  ├── What changes: <one line per file — what specifically changes>
  ├── What stays the same: <functions / sections NOT being touched>
  ├── Risk level: LOW | MEDIUM | HIGH
  └── Rollback: <what to revert if it breaks>

Do not proceed until this plan is written.
If the plan has more than 5 files → split into sub-upgrades.
Complete sub-upgrade 1 fully before starting sub-upgrade 2.

════════════════════════════════════════════
UPGRADE RULE 1 — SNAPSHOT BEFORE YOU TOUCH
════════════════════════════════════════════

For every file you will modify, read these before writing:

  A. The FULL function signature (name + params) of every function in the file
  B. Every window.* global set by this file
  C. Every event listener attached (click, storage, message, etc.)
  D. Every <script> tag that loads this file in any HTML

If you cannot answer A–D → READ MORE before continuing.
Never upgrade a file you have only partially read.

════════════════════════════════════════════
UPGRADE RULE 2 — THE NO-BLUNDER CHECKLIST
════════════════════════════════════════════

These are the top blunders in this repo. Check each one before committing.

BLUNDER 1 — Duplicate declarations
  After every edit: grep -n "const <varName>" <file>
  Each const/let/var must appear exactly once in its scope.
  If you added a new variable → grep for it first.

BLUNDER 2 — Broken script load order
  Any new <script> tag must follow the EXACT loading order:
    1. CDN (Three.js, Chart.js, Supabase)
    2. shared/vd-nav-fix.js
    3. shared/vd-wallet.js
    4. Other shared/*.js
    5. Inline or module scripts — LAST
  Never insert a script tag after an existing inline <script>.

BLUNDER 3 — Touching protected functions
  NEVER modify bodies of: calcPoints, showResult, restart,
  loadQuestion, updateStats, haptic, handleDecision,
  advanceMission, calculateScore, renderCycle, initApp,
  step, draw, reset, spawnFood, selectAnswer, showQuestion,
  startTimer, broadcast, createRoom, joinRoom, commitSwipe,
  finishSession, buildCardElement, startSessionIfEligible,
  hydrateState, syncState, or any rAF callback.
  ADD after them. Never edit inside them.

BLUNDER 4 — Rewriting instead of patching
  If the edit is < 20 lines → use str_replace. One block at a time.
  If the edit is > 20 lines → stop and split into smaller edits.
  Never output a full file when patching an existing one.

BLUNDER 5 — Breaking the IIFE wrapper
  shared/*.js and router.js use IIFE wrappers.
  Every edit must keep the opening (function(global){
  and closing })(window); intact. Check both after editing.

BLUNDER 6 — Removing working code
  Upgrades ADD. They do not remove unless the task explicitly says so.
  If you need to replace a function: keep the old one commented with
  // DEPRECATED <date> — replaced by <newFunctionName>
  Remove it only in a follow-up task explicitly for cleanup.

BLUNDER 7 — Missing guard clauses
  Every new function that reads from window.*, localStorage,
  or Supabase MUST have a guard:
    if (!window.SocialCore) return;
    if (!user || user.is_guest) return;
    try { ... } catch(e) { console.error('<fnName> failed:', e); }
  No naked calls. No assumed availability.

BLUNDER 8 — Hardcoded absolute paths
  All paths must be relative (./path or ../path).
  Never use /tools/... or / as a base. (Breaks GitHub Pages.)

BLUNDER 9 — Adding framework code
  No React. No Vue. No import/export in shared files.
  No npm packages. No build step. No bundler config.
  Vanilla JS only. IIFE or type="module" (only where already used).

BLUNDER 10 — Forgetting README.md update
  If your upgrade changes what a tool does, what its inputs/outputs
  are, or what files it has → update README.md in the same commit.
  README is not optional. It is part of the commit.

════════════════════════════════════════════
UPGRADE RULE 3 — SAFE WIRING PATTERN FOR NEW FEATURES
════════════════════════════════════════════

When wiring a new feature into an existing page:

  STEP 1: Create the new feature in its own function or file first.
          Test that it works in isolation (no side effects).

  STEP 2: Add a feature flag at the top of the file:
            const FEATURE_<NAME>_ENABLED = true;
          Wrap ALL new code in: if (FEATURE_<NAME>_ENABLED) { ... }

  STEP 3: Wire into existing code at exactly ONE callsite.
          Choose the safest callsite (init, DOMContentLoaded,
          or after an existing data-load completes).

  STEP 4: The existing flow must work identically when the flag is false.
          Never make the new feature a hard dependency.

  STEP 5: Only remove the flag in a separate cleanup task.

════════════════════════════════════════════
UPGRADE RULE 4 — SOCIAL LAYER UPGRADES (VIA-specific)
════════════════════════════════════════════

When upgrading shared/social-core.js or anything that calls SocialCore:

  A. Read the full SocialCore public API first:
       window.SocialCore = { createPost, reactToPost, followUser,
         listFeed, listPostsByAuthor, listProfiles, searchProfiles,
         bootstrapProfile, syncProfileCounts, isFollowing, readState }

  B. Never change the signature of existing public methods.
     Add new methods only. Breaking the API breaks every caller.

  C. Every new SocialCore method must:
     - Handle missing state gracefully (readState returns { posts:[], follows:[], reactions:[], users:{} })
     - Write back with writeState() atomically
     - Return a value or null — never throw to callers

  D. When adding new state fields to the storage schema:
     Always provide a default in readState():
       const state = JSON.parse(...);
       state.newField = state.newField || defaultValue;
     This ensures old persisted data doesn't break.

  E. After any SocialCore change, verify these callers still work:
       index.html → getSocialFeedCards()
       index.html → buildDiscover()
       index.html → refreshSocialSurfaces()
       index.html → publishComposerPost()
       index.html → followDiscoverUser()

════════════════════════════════════════════
UPGRADE RULE 5 — INDEX.HTML UPGRADES (SURGICAL ONLY)
════════════════════════════════════════════

index.html is 2100+ lines. It is the most dangerous file in the repo.

Rules:
  - Never read or output the full file. Use line ranges.
  - Find the exact insertion point using a unique string grep.
  - Insert with str_replace using 3–5 lines of context above and below.
  - After every edit: grep -n "<newly added function name>" index.html
    to confirm it landed in the right place.
  - CSS additions go inside the existing <style> block — never a new one.
  - New JS functions go in the JS section, never inside HTML attributes
    beyond onclick="<functionName>()" style calls.

Sections map (approximate — verify with grep before assuming):
  Lines 1–200     → CSS variables, base styles
  Lines 200–470   → Component styles (cards, sheets, nav)
  Lines 470–700   → HTML structure (feed, discover, about, profile)
  Lines 700–1000  → VIA object, auth, user profile functions
  Lines 1000–1200 → Card/feed logic, CARDS array, syncCreatorContent
  Lines 1200–1700 → Sheet engine, buildDiscover, swipe handlers
  Lines 1700–2100 → Event listeners, init, window.* exports

════════════════════════════════════════════
UPGRADE RULE 6 — POST-UPGRADE VERIFICATION
════════════════════════════════════════════

After every upgrade, run ALL of these before declaring done:

  □ node --check <modified .js files>   → zero syntax errors
  □ grep -n "const <newVar>" <file>     → appears exactly once
  □ grep -n "FEATURE_" index.html       → flag exists and wraps new code
  □ grep -n "README" <tool folder>      → README.md exists and is updated
  □ grep -n "import\|require\|npm"      → must return nothing in shared files
  □ grep -n "href=\"/\""                → must return nothing in nav files
  □ All protected functions still present and unchanged (spot check 3)
  □ tools-manifest.json is valid JSON   → python3 -c "import json; json.load(open('tools-manifest.json'))"

If ANY check fails: fix before outputting the completion table.

════════════════════════════════════════════
UPGRADE RULE 7 — OUTPUT FORMAT FOR UPGRADES
════════════════════════════════════════════

After completing an upgrade, output ONLY this table:

  UPGRADE COMPLETE
  ┌─────────────────────────────┬──────────────┬────────────┬──────────┐
  │ File                        │ Change       │ Lines      │ Verified │
  ├─────────────────────────────┼──────────────┼────────────┼──────────┤
  │ shared/social-core.js       │ +listProfiles│ 167–210    │ ✓        │
  │ index.html                  │ buildDiscover│ 1872–1896  │ ✓        │
  │ tools/task-splitter/README  │ created      │ 1–45       │ ✓        │
  └─────────────────────────────┴──────────────┴────────────┴──────────┘

  Skipped: <file> — reason
  Failed checks: none | <list>
  Rollback: revert <file> lines <N>–<M>

No prose. No summary paragraphs. Table only.

════════════════════════════════════════════
UPGRADE RULE 8 — WHEN TO STOP AND ASK
════════════════════════════════════════════

Stop immediately and ask the user if:

  - The upgrade requires touching more than 5 files
  - A protected function needs to change to make the upgrade work
  - The upgrade requires a new shared dependency (new script tag in many files)
  - The storage schema change would break existing persisted user data
  - A check from Rule 6 fails and the fix is non-trivial
  - You are unsure which of two callsites is safer to use

A question saves more tokens than a failed upgrade.
...
LOW RISK (proceed directly):
  - Adding a new tool folder with 4 new files
  - Adding a new method to SocialCore (new name, no signature change)
  - CSS addition inside existing <style> block
  - README.md creation or update
  - New config.json entry

MEDIUM RISK (write plan first, then proceed):
  - Editing index.html JS section
  - Adding a new sheet type to the sheet engine
  - Adding a new window.* export
  - New feature flag + wiring into 1 callsite

HIGH RISK (write plan, wait for user to confirm, then proceed):
  - Any change to SocialCore public method signatures
  - Any change to router.js
  - Any change to shared/shared.css
  - Any change near CARDS array or syncCreatorContentSurface()
  - Any change to script load order
  - Any change to tools-manifest.json existing entries
  - Any change to _redirects
